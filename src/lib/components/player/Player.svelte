...
<div class="bg-player flex w-full h-full rounded-3xl relative flex-grow {handleOrientation(orientation)}">
	<button
		on:mousedown={() => handleMouseDown('subtract')}
		on:mouseup={() => handleMouseUp('subtract')}
		class="w-1/2 flex justify-start items-center active:bg-player-light rounded-l-3xl bg-player"
	>
		<Minus />
	</button>
	<button
		...
		class="w-1/2 flex justify-end items-center active:bg-player-light rounded-r-3xl bg-player"
	>
		<Plus />
	</button>
	<div
	...
	>
		...
		<span class="text-black text-7xl grow h-1/3 flex items-center text-center"
